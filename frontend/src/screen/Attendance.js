...
const MarkAttendanceScreen = ({ navigation }) => {
  ...
  const renderItem = ({ item }) => (
    <View style={{ marginBottom: 10, flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFD95A', width: '100%', height: '75%', borderRadius: 5 }}>
      ...
      <TouchableOpacity style={{ marginRight: 10 }} onPress={() => toggleAttendance(item.id)}>
        <MaterialIcons name={attendance[item.id] ? 'check-box' : 'check-box-outline-blank'} size={30} color={attendance[item.id] ? '#4CAF50' : '#757575'} />
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <View style={{ padding: 20, backgroundColor: '#4C3D3D', flex: 1 }}>
      <TextInput placeholder="Enter the Date" value={date} onChangeText={setDate} style={{ marginBottom: 20, backgroundColor: '#C07F00', height: "8%", borderRadius: 5, padding: 20 }} />
